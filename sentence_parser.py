...
class LtpParser:
    ...
    def format_labelrole(self, output):
        """
        由：
    ...
    def build_parse_child_dict(self, segment, pos, dep):
        ...
        rely_id = dep['head']   #新版中head下即为依存父节点id
        # print(rely_id)
        ...
        for index in range(len(segment)):
            child_dict = {}
            for dep_index in range(len(dep['label'])):
                if dep['head'][dep_index] == index + 1:  # 如果依存父节点是当前词index+1
                    if dep['label'][dep_index] in child_dict:
                        child_dict[dep['label'][dep_index]].append(dep_index)
                    else:
                        child_dict[dep['label'][dep_index]] = []
                        child_dict[dep['label'][dep_index]].append(dep_index)
    ...
    def parser_main(self, sentence):
        print(self.ltp.pipeline([sentence]).to_tuple())
        output = self.ltp.pipeline([sentence])  # 分词
        print(output.dep)
        print(output.sdp)
        segment = output.cws
        pos = output.pos  # 词性标注
        ner = output.ner  # 命名实体识别
        # srl = self.ltp.srl(hidden)  # 语义角色标注
        roles_dict = self.format_labelrole(output)   # 语义角色标注
        dep = output.dep  # 依存句法分析
        # sdp = output.sdp
